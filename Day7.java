...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int count = getHowManyBagsCanContain(ruleSet, "shiny gold");
        System.out.println(count);
        System.out.println(count + " == 139 ? => " + (count == 139));
        
        count = getHowMayBagsAreContainedAt(ruleSet, ruleSet.types.indexOf("shiny gold"));
        System.out.println(count);
    }
    
    public static int getHowManyBagsCanContain (RuleSet ruleSet, String matchingType) {
        int matchingIndex = ruleSet.types.indexOf(matchingType);
        
        if (matchingIndex < 0) {
            return 0;
        }
        
        boolean[] matches = new boolean[ruleSet.types.size()];
        for (int i = 0; i < matches.length; i++) {
            matches[i] = ruleSet.adyacencyMatrix[i][matchingIndex] > 0;
        }
        ...
        for (int k = 0; k < matches.length; k++) {
            for (int i = 0; i < matches.length; i++) {
                ...
                for (int j = 0; j < matches.length; j++) {
                    if (ruleSet.adyacencyMatrix[i][j] > 0 && matches[j]) {
                        matches[i] = true;
                        break;
                    }
                }
    ...
    public static int getHowMayBagsAreContainedAt (RuleSet ruleSet, int index) {
        int count = getContainedTypesCount(ruleSet, index);
        
        if (count > 0) {
            int sum = 0;
            
            for (int i = 0; i < ruleSet.types.size(); i++) {
                sum += ruleSet.adyacencyMatrix[index][i] * getHowMayBagsAreContainedAt(ruleSet, i);
            }
    ...
    public static int getContainedTypesCount (RuleSet ruleSet, int index) {
        int count = 0;
        
        for (int i = 0; i < ruleSet.types.size(); i++) {
            if (ruleSet.adyacencyMatrix[index][i] > 0) {
                count++;
            }
        }
    ...
    public static void tests () {
        ...
        count = getHowMayBagsAreContainedAt(ruleSet, ruleSet.types.indexOf("shiny gold"));
        System.out.println(count + " == 126 ? => " + (count == 126));
...
public class RuleSet {
    ...
    public RuleSet (String[] rules) {
        ...
        for (int i = 0; i < rules.length; i++) {
            String[] data = rules[i].split(" contain ")[0].split(" ");
            types.add(String.format("%s %s", data[0], data[1]));
        }
        
        
        adyacencyMatrix = new int[types.size()][types.size()];
       
        for (int i = 0; i < rules.length; i++) {
            String[] containedBags = rules[i].split(" contain ")[1].split(", ");
            
            for (String containedBag : containedBags) {
                String[] data = containedBag.split(" ");
                    
                if (data[0].equals("no")) {
                    continue;
                }
                    
                String containedType = String.format("%s %s", data[1], data[2]);
                int containedQuantity = Integer.parseInt(data[0]);
                    
                adyacencyMatrix[i][types.indexOf(containedType)] = containedQuantity;
            }
        }
    }
    ...
    public void PrintAdyacencyMatrix () {
        for (int i = 0; i < types.size(); i++) {
            for (int j = 0; j < types.size(); j++) {
                System.out.print("\t" + adyacencyMatrix[i][j]);
            }
            System.out.println();
        }
    }
